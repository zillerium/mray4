...
const VotesSummary: React.FC<VotesSummaryProps> = ({ votesData, nftId, nftOwner }) => {
  const totalVotes = votesData.reduce((acc, vote) => acc + vote.votes, BigInt(0));
  const weightedVotes = votesData.reduce((acc, vote) => acc + vote.votes * vote.price, BigInt(0));

  const weightedAverageEth = totalVotes > BigInt(0)
  ...
  return (
    <div className="mt-4 p-4 bg-gray-100 rounded-lg">
      ...
      <p className="text-sm">
        <span className="font-semibold">Total Weighted Votes (in USD):</span> {weightedAverageEth.toFixed(2)}
      </p>
