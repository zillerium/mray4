...
const GetInvestorBalance: React.FC<{ walletAddress: string }> = ({ walletAddress }) => {
  const [investorBalance, setInvestorBalance] = useState<string | null>(null);

  const { refetch } = useContractRead({
    address: contractAddress,
    abi: usdcTreasuryABI,
    functionName: 'getUsdcInvestorBalance',
    args: [walletAddress],
  });

  const handleGetInvestorBalanceClick = async () => {
    try {
      const result = await refetch();
      if (result.data) {
        const formattedBalance = (Number(result.data) / 10 ** 6).toFixed(2); // Convert to USDC format
        setInvestorBalance(formattedBalance);
      } else {
        setInvestorBalance('0.00');
      }
    } catch (e) {
      console.error('Error fetching USDC Investor balance:', e);
      setInvestorBalance('0.00');
    }
  };
  ...
  return (
    ...
    >
      {/* Title */}
      <span className="text-xl font-bold text-gray-700">USDC Investor Balance:</span>

      {/* Balance */}
      <span className="text-xl font-bold text-gray-900">
        {investorBalance !== null ? `${investorBalance} USDC` : ''}
      </span>

      {/* Button */}
      <button
        onClick={handleGetInvestorBalanceClick}
...
export default GetInvestorBalance;
